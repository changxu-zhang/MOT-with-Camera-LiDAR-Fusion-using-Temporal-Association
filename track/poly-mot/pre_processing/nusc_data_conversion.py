...
def concat_dict_attr(dictbox: dict, *attrs) -> List:
    ...
    for attr in attrs:
        if attr == 'detection_name':
            res += [CLASS_SEG_TO_STR_CLASS[dictbox[attr]]] # here have to use original mapping, otherwise all subsequent mapping should be modified
            continue
...
def dictdet2array(dets: List[dict], *attrs) -> Tuple[List, np.array]:
    dets_wo_untrack_cat = []
    for det in dets:
        if det['detection_name'] in BEVFUSION_ID2CAT:
            det['detection_name'] = [cat for cat, id in BEVFUSION_CAT2ID.items() if id == det['detection_name']][0]
            dets_wo_untrack_cat.append(det)          
    listdets = [concat_dict_attr(det, *attrs) for det in dets_wo_untrack_cat if det['detection_name'] in BEVFUSION_CAT2ID]
...
def arraydet2box(dets: np.array, ids: np.array = None):
    ...
    for idx, det in enumerate(dets):
        curr_box = NuscBox(center=det[0:3], size=det[3:6], rotation=det[8:12],
                            ...
                            name=CLASS_STR_TO_SEG_CLASS[int(det[13])]) # here also need to use original mapping ref. line 30
