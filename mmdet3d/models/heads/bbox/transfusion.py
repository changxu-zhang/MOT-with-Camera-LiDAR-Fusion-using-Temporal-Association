...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        for i in range(self.num_decoder_layers):
            ...
            self.save_tensor(query_feat, metas[0]['token']+'_bev_after_transfusion.pt')
    ...
    def get_bboxes(self, preds_dicts, metas, img=None, rescale=False, for_roi=False):
        ...
        assert len(rets) == 1 # batch_size is 1 due to setting in line 104 in mmdet3d/apis/train.py
        assert len(rets[0]) == batch_size # 1
        ...
        res = [
            ...
            # [
            #     metas[1]["box_type_3d"](
            #         rets[0][1]["bboxes"], box_dim=rets[0][1]["bboxes"].shape[-1]
            #     ),
            #     rets[0][1]["scores"],
            #     rets[0][1]["labels"].int(), # keep the last one in the queue
            # ],
            # [
            #     metas[2]["box_type_3d"](
            #         rets[0][2]["bboxes"], box_dim=rets[0][2]["bboxes"].shape[-1]
            #     ),
            #     rets[0][2]["scores"],
            #     rets[0][2]["labels"].int(),
            # ],
            # [
            #     metas[3]["box_type_3d"](
            #         rets[0][3]["bboxes"], box_dim=rets[0][3]["bboxes"].shape[-1]
            #     ),
            #     rets[0][3]["scores"],
            #     rets[0][3]["labels"].int(),
            # ]
